 ...
 */
    bool MacronTable::read(const std::string &fname) {
        ...
        while (!ifs.eof()) {
            ...
            if (!convertTable.insert(std::make_pair<std::vector<std::string>, Result *>(static_cast<std::vector<std::basic_string<char>, std::allocator<std::basic_string<char>>> &&>(pl), reinterpret_cast<Result *&&>(result))).second) {
