 ...
 */
    bool PhonemeTable::read(const std::string &fname) {
        ...
        while (!ifs.eof()) {
            ...
            if (false == convertTable.insert(std::make_pair<std::string, PhonemeList *>(st.at(0), reinterpret_cast<std::vector<std::basic_string<char>, std::allocator<std::basic_string<char>>> *&&>(pl))).second) {
